...
protected:
	...
	YsShellExt mesh;
	std::vector <float> vtx,nom,col,sliceVtx,rayItscVtx;
	YsVec3 bbx[2];
	YsVec3 polygonNormal;

	std::vector<YsShellExt::PolygonHandle> polyg;
...
public:
	...
	// void DFS(YsShellExt::PolygonHandle plHd, std::vector<YsShellExt::PolygonHandle>& polyg)
	// {
	// 	auto nEdge = mesh.GetPolygonNumVertex(plHd);
	// 	for(int i = 0; i < nEdge; ++i)
	// 		{
	// 			auto neigHd = mesh.GetNeighborPolygon(plHd,i);
	// 			auto angle = mesh.ComputeDihedralAngle(plHd,neigHd);
	// 			std::cout << angle;
	// 			if((angle < 20)) 
	// 			{
	//     			std::vector<YsShellExt::PolygonHandle>::iterator iter;
	//     			if(std::find(polyg.begin(),polyg.end(),neigHd) != polyg.end())
	//     			{
	//     				continue;
	//     			}
	// 			} 
...
/* virtual */ void FsLazyWindowApplication::Interval(void)
{
	...
	if(evt==FSMOUSEEVENT_LBUTTONDOWN)
	{
		...
		if(mesh.NullPolygon()!=plHd)
		{
			...
			RemakeVertexArray();
			//void(plHd,&poly);
			//polygonNormal = mesh.GetNormal(plHd);
			//auto nEdge = mesh.GetPolygonNumVertex(plHd);
			//for(int i = 0; i < nEdge; ++i)
			//{
			//	auto neigHd = mesh.GetNeighborPolygon(plHd,i);
			//	mesh.SetPolygonColor(neigHd,YsGreen());
			//}
			//double angle;
			//RemakeVertexArray();
		}
	...
	{
		count = 3;
		rayItscVtx = MakeSliceVertexArray(mesh);
	}
...
/* virtual */ void FsLazyWindowApplication::Draw(void)
{
	...
	{
		glEnableClientState(GL_VERTEX_ARRAY);
		glVertexPointer(3, GL_FLOAT, 0, rayItscVtx.data());
		glDrawArrays(GL_POINTS, 0, rayItscVtx.size() / 3);
		glPointSize(4);
		glDisableClientState(GL_VERTEX_ARRAY);
	}
